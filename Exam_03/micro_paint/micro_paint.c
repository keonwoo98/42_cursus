...
int
	canva_dim_check(FILE *file, t_canva *canva_dim)
{
	int		ret;
...
void
	shape_draw(char **buffer, t_shape shape_dim, t_canva canva)
{
	int		x;
	int		y = 0;
	...
	if (shape_dim.type == 'R')
	{
		while (y < canva.height)
		{
			...
			while (x < canva.width)
			{
				if (x >= shape_dim.x && x <= shape_dim.x + shape_dim.width && y >= shape_dim.y && y <= shape_dim.y + shape_dim.height)
...
int
	final_draw_check(t_shape *shape_dim, FILE *file, char **buffer, t_canva canva_dim)
{
	int		ret;
	...
	main(int argc, char **argv)
{
	...
	if (!canva_dim_check(file, &canva_dim))
	{
		write(1, "Error: Operation file corrupted\n", 32);
		fclose(file);
		return (1);
